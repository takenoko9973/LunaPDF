...
アイコンボタンは28論理pt、アイコンは20論理ptで、ページ入力欄と検索欄には共通の明示高さがなかった。親パネル、アイコン、入力欄の寸法決定が揃っていなかった。
...
4. 進行軸上の隣接ギャップが、隣接する二つのグリフの大きい方の寸法を超えた場合は、段組みまたは無関係な領域として分割する。
5. 統合後は軸平行の外接矩形を作らず、区間両端グリフの外縁を再利用する。これにより傾いた文字や回転文字の外縁を維持する。
...
- Windows GNU debug cross build
  - コンパイル成功
  - 既存 `dist/lunapdf-debug.exe` は実行中プロセスに保持されていて置換できなかったため、同じビルド成果物を `dist/lunapdf-annotation-ui-debug.exe` として配置した。

### PDF外部検査

- 新コードで `acceptance-highlight-260728.pdf` を生成した。
- qpdf
